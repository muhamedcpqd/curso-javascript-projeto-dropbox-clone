class DropboxController {
    ...
    getOutsideClick() {
        window.addEventListener('click', e => {
            if (!document.getElementById('list-of-files-and-directories').contains(e.target)) {
        ...
        this.inputFilesEl.addEventListener('change', event => {
            ...
            this.uploadTask(event.target.files).then(responses => {
                ...
                for(let i=0; i < responses.length; i++) {
                    let dataArray = responses[i];

                    this.getFirebaseRef().push().set({
                        name: dataArray[1].name,
                        type: dataArray[1].contentType,
                        path: dataArray[0],
                        size: dataArray[1].size
                    });
                }
    ...
    connectFirebase() {
        // Initialize Firebase
        var config = {
            apiKey: "AIzaSyDlVlqdSe9rK_MmxDrssK-hJZGNmoz2ni0",
            authDomain: "dropbox-clone-e9347.firebaseapp.com",
            databaseURL: "https://dropbox-clone-e9347.firebaseio.com",
            projectId: "dropbox-clone-e9347",
            storageBucket: "dropbox-clone-e9347.appspot.com",
            messagingSenderId: "1019422368318"
        };
    ...
    uploadTask(files) {
        ...
        [...files].forEach(file => {
            ...
            promises.push(new Promise((resolve, reject) => {
                ...
                task.on('state_changed', snapshot_onload => {

                    this.uploadedProgress({
                        loaded: snapshot_onload.bytesTransferred,
                        total: snapshot_onload.totalBytes
                    }, file);
                    ...
                    fileRef.getDownloadURL().then(downloadedURL => {
                        Data.push(downloadedURL);
